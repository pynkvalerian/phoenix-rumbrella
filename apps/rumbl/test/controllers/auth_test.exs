defmodule Rumbl.AuthTest do
  ...
  setup %{conn: conn} do
    conn =
      conn
      # pass router and browser pipelines so we have session and flash support
  ...
  test "login puts the user in the session", %{conn: conn} do
    login_conn =
      ...
      |> send_resp(:ok, "") # send resp to client with {status, body}

    # create new req to see if session[:user_id] persists
